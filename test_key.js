// test_key.js (CORRECTED to use the consistent GEMINI_API_KEY)
...
async function runTest() {
  ...
  // This now matches the variable used in server.js for a consistent and valid test.
  const apiKey = process.env.GEMINI_API_KEY;

  // 2. VERY IMPORTANT: Check if the key was loaded at all.
  if (!apiKey) {
    // *** CORRECTED ERROR MESSAGE ***
    console.error("\nERROR: API Key not found in .env file! `process.env.GEMINI_API_KEY` is undefined.");
    console.log("Please ensure your .env file exists and contains: GEMINI_API_KEY=YourActualKey\n");
    return;
  }
  ...
  try {
    ...
    console.error("Full Error Details:", error.message); // Log just the message for clarity
  } finally {
      console.log("\n--- Test Finished ---");
  }
}
